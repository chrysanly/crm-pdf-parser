...
| resume_template | string(30) | NOT NULL, default `classic` | enum `ResumeTemplate` |
| section_order | json | nullable | `list<string>` overriding the template default |
...
**Relations:** hasMany resumes (cascade on delete).
...
**Relations:** belongsTo company, belongsTo uploader (users).
